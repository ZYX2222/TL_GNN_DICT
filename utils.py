...
class LoadhERGDataset(Dataset):
    ...
    def process(self):
        ...
        for idx, row in tqdm(self.data.iterrows(), total=self.data.shape[0]):
            ...
            data = f.to_pyg_graph()
            data.y = self._get_label(row["hERG"])
...
class LoadDICTDataset(Dataset):
    def __init__(self, root, raw_filename, transform=None, pre_transform=None):
        ...
        """
        self.raw_filename = raw_filename
        super(LoadDICTDataset, self).__init__(root, transform, pre_transform)
...
class LoadSMILESDataset(Dataset):
    def __init__(self, root, raw_filename, transform=None, pre_transform=None):
        self.raw_filename = raw_filename
        super(LoadSMILESDataset, self).__init__(root, transform, pre_transform)
